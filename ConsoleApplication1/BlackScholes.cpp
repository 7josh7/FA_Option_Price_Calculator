...
double BlackScholes::gamma() const {
    return std::exp(-y_ * time_) * myDStat.NormDist(d1) / (S_ * sigma_ * time_sqrt);
}
double BlackScholes::vega() const {
    return S_ * std::exp(-y_ * time_) * myDStat.NormDist(d1) * time_sqrt;
}
double BlackScholes::theta() const {
	return -S_ * sigma_ * std::exp(-y_ * time_) * n_d1 / (2 * time_sqrt) + y_ * S_ * std::exp(-y_ * time_) * n_d1 - r_ * K_ * std::exp(-r_ * time_) * n_d2;
}
